...
void TM_GPIO_SetPinAsInput(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, TM_GPIO_PuPd_t GPIO_PuPd) {
#ifdef STM32F1xx
	TM_GPIO_INT_Init( GPIOx, GPIO_Pin, TM_GPIO_Mode_IN, TM_GPIO_OType_OD, GPIO_PuPd, TM_GPIO_Speed_High );
#else
	uint8_t i;
	/* Go through all pins */
	for (i = 0x00; i < 0x10; i++) {
...
void TM_GPIO_SetPinAsOutput(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, TM_GPIO_OType_t GPIO_OType) {
#ifdef STM32F1xx
	TM_GPIO_INT_Init( GPIOx, GPIO_Pin, TM_GPIO_Mode_OUT, GPIO_OType, TM_GPIO_PuPd_NOPULL, TM_GPIO_Speed_High );
#else
	uint8_t i;
	/* Go through all pins */
	for (i = 0x00; i < 0x10; i++) {
...
void TM_GPIO_INT_Init(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, TM_GPIO_Mode_t GPIO_Mode, TM_GPIO_OType_t GPIO_OType, TM_GPIO_PuPd_t GPIO_PuPd, TM_GPIO_Speed_t GPIO_Speed) {
	...
	/* STM32F0xx and STM32F1xx series does not have FAST speed mode available */
	...
	switch( GPIO_Mode ){
	case TM_GPIO_Mode_IN: /*!< GPIO Pin as General Purpose Input */
		GPIO_Init.Mode = GPIO_MODE_INPUT;
		break;
	case TM_GPIO_Mode_OUT: /*!< GPIO Pin as General Purpose Output */
		if( GPIO_OType == TM_GPIO_OType_PP)
			GPIO_Init.Mode = GPIO_MODE_OUTPUT_PP;
		else
			GPIO_Init.Mode = GPIO_MODE_OUTPUT_OD;
		break;
	case TM_GPIO_Mode_AF:  /*!< GPIO Pin as Alternate Function */
		if( GPIO_PuPd == TM_GPIO_PuPd_UP )
			GPIO_Init.Mode = GPIO_MODE_AF_PP;
		else
			GPIO_Init.Mode = GPIO_MODE_AF_OD;
		break;
	case TM_GPIO_Mode_AN:  /*!< GPIO Pin as Analog input/output */
		GPIO_Init.Mode = GPIO_MODE_ANALOG;
		break;
	}
	switch( GPIO_Speed ){
	case TM_GPIO_Speed_Low: GPIO_Init.Speed = GPIO_SPEED_LOW; break;
	case TM_GPIO_Speed_Medium: GPIO_Init.Speed = GPIO_SPEED_MEDIUM; break;
	default: GPIO_Init.Speed = GPIO_SPEED_HIGH; break;
	}
