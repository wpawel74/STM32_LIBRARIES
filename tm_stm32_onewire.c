...
void TM_OneWire_Init(TM_OneWire_t* OneWireStruct, GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin) {
	...
	/* Init GPIO pin */
	TM_GPIO_Init(GPIOx, GPIO_Pin, TM_GPIO_Mode_OUT, TM_GPIO_OType_OD, TM_GPIO_PuPd_NOPULL, TM_GPIO_Speed_High);
	...
	if((GPIO_Pin & (uint32_t)0x00FF) > 0){
		OneWireStruct->m_Register = &GPIOx->CRL;
		...
		for (pinpos = 0x00; pinpos < 0x08; pinpos++) {
			...
			if (currentpin == pos){
				RegShift = (pinpos*4);
				OneWireStruct->m_RegMask = ((uint32_t)0x0F) << (pinpos*4);
				break;
			}
		}
	} else {
		OneWireStruct->m_Register = &GPIOx->CRH;
	...
	OneWireStruct->m_InputMask = (((GPIO_Mode_IN_FLOATING) << RegShift) & OneWireStruct->m_RegMask);
	OneWireStruct->m_OutputMask = (((uint32_t)GPIO_Mode_Out_OD|(uint32_t)GPIO_Speed_50MHz) << RegShift) & OneWireStruct->m_RegMask;
}

void TM_OneWire_SetInput(TM_OneWire_t* OneWireStruct){
	*OneWireStruct->m_Register &= ~OneWireStruct->m_RegMask;
	//*OneWireStruct->m_Register |= (((GPIO_Mode_IN_FLOATING) << OneWireStruct->m_RegShift) & OneWireStruct->m_RegMask);
	*OneWireStruct->m_Register |= OneWireStruct->m_InputMask;
}

void TM_OneWire_SetOutput(TM_OneWire_t* OneWireStruct){
	*OneWireStruct->m_Register &= ~OneWireStruct->m_RegMask;
	//*OneWireStruct->m_Register |= ((((uint32_t)GPIO_Mode_Out_OD|(uint32_t)GPIO_Speed_50MHz) << OneWireStruct->m_RegShift) & OneWireStruct->m_RegMask);
	*OneWireStruct->m_Register |= OneWireStruct->m_OutputMask;
}

void TM_OneWire_WriteHigh(TM_OneWire_t* OneWireStruct){
	OneWireStruct->GPIOx->BSRR = OneWireStruct->GPIO_Pin;
}

void TM_OneWire_WriteLow(TM_OneWire_t* OneWireStruct){
	OneWireStruct->GPIOx->BRR = OneWireStruct->GPIO_Pin;
}
